...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['register'])) {
    ...
    if (empty($organization_name) || empty($email) || empty($password) || empty($confirm_password)) {
        ...
        if ($stmt->num_rows > 0) {
            ...
            $status = 'pending'; // New users are 'pending' until approved by admin (for recipients/admins, or just active for donors)
            ...
            if ($stmt->execute()) {
                $message = '<div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-md mb-4" role="alert">Registration successful! Please login.</div>';
                // Automatically log in the user after successful registration for simplicity in this demo
                ...
                header('Location: donor-dashboard.php'); // Redirect to donor dashboard for demo
                exit();
            } else {
                $message = '<div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-md mb-4" role="alert">Error during registration. Please try again.</div>';
            }
        }
        $stmt->close();
        $conn->close();
    ...
    if (empty($email) || empty($password)) {
        ...
        if ($stmt->num_rows == 1) {
            ...
            if (password_verify($password, $password_hash)) {
                ...
                // Redirect based on user role (you can expand this logic)
                switch ($role) {
                    case 'donor':
                    ...
                    case 'admin':
                        ...
                        break;
                    case 'moderator':
                        // header('Location: moderator-dashboard.php'); // Create a moderator dashboard if needed
                        header('Location: admin-panel.php'); // For now, redirect to admin panel
                        break;
                    default:
                        header('Location: login-register.php'); // Fallback
                        break;
                }
                exit();
            } else {
                $message = '<div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-md mb-4" role="alert">Invalid password.</div>';
            }
        } else {
